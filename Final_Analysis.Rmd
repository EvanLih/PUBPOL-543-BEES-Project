...
#### Importing Data:
We are importing our for analysis. Our dataset was merged and cleaned in R. From this point on, the code will be hidden for clarity. Please see <https://github.com/EvanLih/PUBPOL-543-BEES-Project/raw/master/Final_Analysis.Rmd> for the full code. 
...
<center> <h4>  Plotting Electricity Access *(Density Plot)* </h4> </center>
...
```
<br>
<p> **Main Findings:**- The density plot a longer tail to the left, which means that the majority of countries have high accessibility to electricity.
...
```
<br>
<p> **Main Findings:** The highest point of curve is located at around 10%, which means a great number of countries have a low renewable consumption level.
</p>
<br>
<center> <h4>  Plotting Land Use *(Grouped Bar Chart)* </h4> </center>
...
<br>
<center> <h4>  Plotting Income *(Bar Chart)* </h4> </center>
<center>**A summary of the number of countries with different income levels in each continent.**</center>
```{r echo=FALSE, fig.align='center', message=FALSE, warning=FALSE}
...
```
<br>
<p> **Main Findings:** :In general across all Continents, it seems that as Access to Electricity Increases, SDI also increases. However, there are a few key excepotions to this. Almost all countries in Europe have a very high Access to Electricity, but widly varying levels of SDI. 
</p>
...
```
<br>
<p> **Main Findings:** After looking at countries who have access to electricity from 90 to 100%, its clear how the different Continents rank. To name a few key points, South America has a high access to electricity while having a high SDI, while Europe has a high access to electricity while have a relatively low SDI. 
...
<center><h4> by Country *(Hexagonal Heatmap)*</h4></center>
```{r echo=FALSE, fig.align='center', message=FALSE, warning=FALSE}
...
<center><h4> by Continent *(Scatterplot)*</h4></center>
```{r echo=FALSE, fig.align='center', message=FALSE, warning=FALSE}
...
```
<center><h3> Original SDI Index</h3></center>
```{r echo=FALSE, fig.align='center', message=FALSE, warning=FALSE}
...
```
<p> **Main Findings:** Note: Areas on the map with Grey indicate lack of data.
This shows the existing SDI scoring system (higher is better). We chose to unify SDI scores by continent, as to make it clearer to differentiate. The Americas seem to score highly (particularly South America). while large parts of Asia (Russia and China) scoor not so well. 
</p>
<center><h3> New SDI SDI Index</h3></center>
```{r echo=FALSE, fig.align='center', message=FALSE, warning=FALSE}
...
```
<p> **Main Findings:** Note: Areas on the map with Grey indicate lack of data.
Our new index includes these key variables: 
- Resource REnt 
- Forest: Forest area (% of land area) by country
- Agriculture (% of land area) by country
- Access To Electricity (% of population)
